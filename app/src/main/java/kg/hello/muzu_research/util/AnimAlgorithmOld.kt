...
class AnimAlgorithmOld(muzUFile: InputStream) {
    private val notes: List<Note>
    init {
        val muzU = MuzU(muzUFile)
        notes = muzU.tracks.flatMap { t -> t.notes }.sortedWith(compareBy { n -> n.time })
    }
...
//        var b = (minKeyClr.blue*minClrMultiplier+maxKeyClr.blue*maxClrMultiplier).toInt()
    ...
    private fun getCurInd(dur: Double): Int{
        return if(dur < notes[0].time) {
            -1
        } else if(notes[lastInd].time <= dur && dur < notes[lastInd+1].time){
            lastInd
        }else if(notes[lastInd+1].time <= dur && dur < notes[lastInd+2].time){
            lastInd++
            lastInd+1
        }else{
